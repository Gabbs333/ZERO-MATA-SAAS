-- Script de simulation COMPLÈTE avec ID explicite
-- On contourne le problème de valeur par défaut manquante pour tester le reste (Triggers/RLS)
...
BEGIN
    ...
    -- Générer un ID explicite pour le test
    v_session_id := gen_random_uuid();

    RAISE NOTICE '---------------------------------------------------';
    RAISE NOTICE 'TEST 1: INSERTION SESSION (ID explicite)';
    ...
    EXCEPTION WHEN OTHERS THEN
        ...
        -- Si ça échoue ici avec ID fourni, c'est un problème de droits/RLS
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'Erreur nettoyage (pas grave): %', SQLERRM;
    END;
