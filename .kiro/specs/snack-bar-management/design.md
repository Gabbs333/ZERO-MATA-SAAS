...
Le système adopte une architecture serverless basée sur Supabase avec synchronisation en temps réel native.
...
graph TB
    ...
    AS1 <-->|HTTPS + Realtime| AUTH
    AS2 <-->|HTTPS + Realtime| AUTH
    ASN <-->|HTTPS + Realtime| AUTH
    SC <-->|HTTPS + Realtime| AUTH
    TBP <-->|HTTPS + Realtime| AUTH
...
1. **Zéro infrastructure à gérer**
   - Pas de serveur backend à coder
   - Pas de déploiement à configurer
   - Pas de maintenance DevOps

2. **API auto-générée**
   - REST API générée depuis le schéma PostgreSQL
   - GraphQL disponible si besoin
   - Authentification intégrée

3. **Realtime natif**
   - Subscriptions PostgreSQL natives
   - Pas besoin de Socket.io
   - Synchronisation automatique

4. **Row Level Security (RLS)**
   - Permissions au niveau base de données
   - Sécurité garantie même si le client est compromis
   - Pas de middleware à coder

5. **Offline-first possible**
   - Cache local avec IndexedDB
   - Synchronisation automatique
   - Mode dégradé si Internet tombe
...
### Mapping Complet des Écrans UI aux Ressources Supabase

Un document détaillé de mapping a été créé pour établir la correspondance exacte entre les 18 écrans de maquettes HTML (UI_mock_up.md) et les ressources backend Supabase. Ce mapping couvre :

- **Tables Supabase** : Les 12 tables utilisées par chaque écran
- **Requêtes Supabase** : Exemples de code TypeScript pour chaque opération
- **Fonctions PostgreSQL** : Les 10 fonctions RPC appelées
- **Triggers Automatiques** : Les 15+ triggers qui s'exécutent automatiquement
- **Subscriptions Realtime** : Les 8 channels principaux pour la synchronisation temps réel
- **RLS Policies** : Les 30+ policies de sécurité appliquées

**Référence** : Voir le document complet `FRONTEND_BACKEND_MAPPING.md` pour tous les détails d'implémentation.

### Résumé des 18 Écrans et Leurs Ressources Principales

**Application Manager (Gérant)** :
1. **Manager Dashboard** - KPIs, graphiques CA/bénéfices (`get_kpis`, `analytics_evolution_ca`)
2. **Stock Inventory** - Gestion stock avec alertes (`stock`, `check_stock_alerts`)
3. **Financial Dashboard** - CA vs Encaissements, créances (`analytics_ca_encaissements`)
4. **Payment Entry** - Enregistrement encaissements (`create_encaissement`)
5. **Supply History** - Historique ravitaillements (`ravitaillements`, `ravitaillement_items`)

**Application Patron (Owner)** :
6. **Profit & Loss** - Rapport profits et pertes (`get_kpis`, calcul bénéfice)
7. **Outstanding Debts** - Créances et factures impayées (`factures_overdue`, `get_factures_impayees_alerts`)

**Application Comptoir (Counter)** :
8. **Counter Validation** - Validation commandes (`validate_commande`, Realtime commandes)
9. **Product Catalog** - Catalogue produits (`get_produits_disponibles`)

**Application Serveuse (Waitress)** :
10. **Order Entry** - Prise de commande (`create_commande`, `get_produits_disponibles`)
11. **Table Management** - Gestion tables (`tables`, mise à jour statuts)

**Shared Screens (Toutes Applications)** :
12. **User Management** - Gestion utilisateurs (Admin API, `profiles`)
13. **Product Editor** - Création/modification produits (`produits`, triggers)
14. **Supply Entry** - Enregistrement ravitaillements (`create_ravitaillement`)
15. **Audit Log** - Historique actions (`audit_logs`)
16. **User Profile** - Profil utilisateur (`profiles`, Auth API)
17. **System Activity** - Activité temps réel (Realtime multi-tables)
18. **Login Screen** - Authentification (Supabase Auth)

### Patterns d'Intégration Communs

**1. Chargement Initial avec Cache**
```typescript
// Hook personnalisé avec TanStack Query
function useSupabaseQuery<T>(table: string, select: string) {
  return useQuery({
...
```typescript
// Hook personnalisé pour Realtime
function useRealtimeSubscription(table: string, callback: Function) {
  useEffect(() => {
...
**3. Mutation avec Optimistic Update**
```typescript
// Hook personnalisé pour mutations
function useSupabaseMutation(mutationFn: Function) {
...
**4. Mode Offline avec Queue Locale**
```typescript
// Queue pour opérations offline
class OfflineQueue {
  private queue: Array<{ id: string, operation: Function }> = []
  
  async add(operation: Function) {
    this.queue.push({ id: uuid(), operation })
    await this.sync()
  }
...
- **Supabase Auth** : Authentification managée avec JWT
- **Durée de vie** : 1 heure (access token) + 7 jours (refresh token)
- **Refresh automatique** : Géré par le client Supabase
- **Stockage** : 
  - Mobile : Secure Storage natif (Keychain iOS / Keystore Android)
  - Web : HttpOnly cookies (géré par Supabase)

**Hashage des Mots de Passe**

- Algorithme : **bcrypt** avec salt rounds = 10 (géré par Supabase)
- Politique de mots de passe : Minimum 8 caractères (configurable)

**Contrôle d'Accès Basé sur les Rôles (RBAC) avec Row Level Security**

Supabase utilise PostgreSQL Row Level Security (RLS) pour les permissions au niveau base de données.

```sql
-- Exemple de politique RLS pour les commandes
-- Les serveuses ne voient que leurs propres commandes
...
CREATE POLICY "comptoir_pending_commandes" ON commandes
  FOR SELECT
  USING (
    auth.jwt() ->> 'role' = 'comptoir'
...
-- Le patron voit tout
CREATE POLICY "patron_all_commandes" ON commandes
  FOR ALL
  USING (
    auth.jwt() ->> 'role' IN ('patron', 'gerant')
  );
```
...
| **Comptoir** | Valider/Rejeter commandes, Voir stock en temps réel, Enregistrer encaissements |
...
1. **Sécurité au niveau base de données** : Même si le client est compromis, les données sont protégées
2. **Pas de middleware à coder** : Les permissions sont déclaratives en SQL
3. **Performance** : Filtrage au niveau PostgreSQL (très rapide)
4. **Audit automatique** : Toutes les requêtes sont loggées par Supabase
...
**Supabase Cloud** : Plateforme Backend-as-a-Service complète
- **PostgreSQL 15+** : Base de données relationnelle managée
- **Supabase Auth** : Authentification JWT + RBAC intégrée
- **Supabase Realtime** : Synchronisation temps réel via PostgreSQL subscriptions
- **Supabase Storage** : Stockage des exports PDF/CSV
- **Edge Functions** : Logique métier custom (Deno runtime)
- **Row Level Security (RLS)** : Permissions au niveau base de données
...
- ❌ Backend Node.js custom : Trop complexe sans DevOps
- ❌ Firebase : Moins adapté pour données relationnelles complexes
- ❌ AWS Amplify : Plus complexe que Supabase
...
**Gestion d'État**
- **Zustand** : Gestion d'état simple et performante (alternative légère à Redux)
- **TanStack Query (React Query)** : Cache et synchronisation avec Supabase
...
**Tests**
- **Vitest** : Tests unitaires (plus rapide que Jest)
...
**Hébergement Backend**
- **Supabase Cloud** : Backend complet managé
  - Free tier : 500 MB database, 2 GB bandwidth, 50 MB storage
  - Pro tier : 8 GB database, 250 GB bandwidth, 100 GB storage (~25 USD/mois)

**Hébergement Frontend**
- **Vercel** : Hébergement des applications React (gratuit pour usage modéré)
- Alternative : Netlify, Cloudflare Pages

**CI/CD**
- **GitHub Actions** : Pipeline CI/CD automatisé
- Alternative : GitLab CI, Vercel CI

**Monitoring et Logging**
- **Supabase Dashboard** : Monitoring intégré
- **Sentry** : Monitoring d'erreurs frontend (optionnel)
- **LogRocket** : Session replay pour debugging (optionnel)
...
**Déploiement**
- **Pas de Docker** : Tout est managé par Supabase et Vercel
- **Pas de serveur** : Architecture serverless complète
- **Pas de configuration** : Déploiement automatique via Git push
...
- Accès distant natif pour le patron
- Backups automatiques quotidiens
- Monitoring et alertes intégrés
- Scalabilité automatique

**Infrastructure**
- **Backend** : Supabase Cloud (tout managé)
- **Frontend** : Vercel (déploiement automatique)
- **Réseau** : WiFi existant du snack-bar
- **Pas de serveur local** : Tout est dans le cloud

**Configuration minimale**

```yaml
# supabase/config.toml (généré automatiquement)
[api]
enabled = true
port = 54321
schemas = ["public"]
...
**Coût Estimé**
- **Free tier** : 0 USD/mois (suffisant pour 1-2 snack-bars en test)
- **Pro tier** : 25 USD/mois par projet (recommandé pour production)
- **Scaling** : +10 USD par 2 GB database supplémentaire
...
// Queue locale pour commandes offline
class OfflineQueue {
  private queue: Command[] = []
  
  async addCommand(command: Command) {
    // Stocker localement
    await AsyncStorage.setItem(`cmd_${command.id}`, JSON.stringify(command))
  ...
  async sync() {
    ...
    for (const command of this.queue) {
      try {
        await supabase.from('commandes').insert(command)
        // Supprimer de la queue si succès
        await AsyncStorage.removeItem(`cmd_${command.id}`)
...
**2. Backup 4G**
- Routeur 4G avec carte SIM (~50 USD one-time)
- Bascule automatique si WiFi principal tombe
- Coût data : ~10-20 USD/mois
- Recommandé pour zones avec Internet instable

**3. Cache agressif**
```typescript
// Configuration TanStack Query pour cache optimal
...
function ConnectionStatus() {
  ...
  return (
    <Alert severity="warning">
      Mode hors-ligne activé. Les données seront synchronisées automatiquement.
    </Alert>
  )
}
```
...
1. **Backup Automatique Quotidien (Supabase Pro)**
   - Dump PostgreSQL complet automatique
   - Rétention : 7 jours (Pro tier) ou 30 jours (Team tier)
   - Point-in-time recovery disponible

2. **Backup Manuel Supplémentaire (Recommandé)**
   ```bash
   # Script de backup hebdomadaire vers stockage externe
   supabase db dump > backup_$(date +%Y%m%d).sql
   # Upload vers Google Drive / Dropbox
   ```

3. **Export des données critiques**
   - Export CSV quotidien des commandes et encaissements
   - Stockage dans Supabase Storage + copie locale
   - Accessible via le tableau de bord patron

**Plan de Récupération**

1. **Panne Supabase** : Rare, mais si ça arrive :
   - Mode offline activé automatiquement
   - Données en queue locale
   - Synchronisation automatique au retour
   - Temps de récupération : < 5 minutes

2. **Corruption Base de Données** : 
   - Restauration depuis dernier backup Supabase
   - Via dashboard Supabase (quelques clics)
   - Temps de récupération : < 30 minutes

3. **Perte de Données** : 
   - Point-in-time recovery avec Supabase Pro
